...
def evaluate_retrieval_metric(candidates, gts, predict_scores=None, top_k=None):
    metric_scores = {}
    ...
    recall = Recall(top_k)
    f_measure = FMeasure(top_k)
    hit_at_k = HitRatio(top_k)
    hit_at_1 = HitRatio(1)
    #ndcg = NDCG()
    ...
    for candidate in candidates:
        if candidate not in candidate2id:
            candidate2id[candidate] = idx
            idx += 1
        candidates_ids.append(candidate2id[candidate])

    for gt in gts:
        if gt not in candidate2id:
            print("Error, ground truth does not include in the candidates.")
            print(gt)
            break

        gt_id = candidate2id[gt]

        if gt_id not in gt_ids:
            gt_ids.append(gt_id)

    if len(gt_ids) > 0:
        candidates_ids = np.asarray(candidates_ids)
        gt_ids = np.asarray(gt_ids)

        mrr_score = mrr.compute(gt_pos=gt_ids, pd_rank=candidates_ids)
        precision_score = precision.compute(gt_pos=gt_ids, pd_rank=candidates_ids)
        recall_score = recall.compute(gt_pos=gt_ids, pd_rank=candidates_ids)
        f_measure_score = f_measure.compute(gt_pos=gt_ids, pd_rank=candidates_ids)
        hit_score = hit_at_k.compute(gt_pos=gt_ids, pd_rank=candidates_ids)
        hit_1_score = hit_at_1.compute(gt_pos=gt_ids, pd_rank=candidates_ids)
        #ndcg_score = ndcg.compute(gt_pos=gt_ids, pd_rank=candidates_ids)

        metric_scores['mrr'] = mrr_score
        metric_scores['precision'] = precision_score
        metric_scores['recall'] = recall_score
        metric_scores['f1'] = f_measure_score
        metric_scores['hit'] = hit_score
        metric_scores['hit1'] = hit_1_score
        #metric_scores['NDCG'] = ndcg_score

        if predict_scores is not None:
            predict_scores = np.asarray(predict_scores)

            map_score = mean_ap.compute(item_indices=candidates_ids, pd_scores=predict_scores, gt_pos=gt_ids)
            auc_score = auc.compute(item_indices=candidates_ids, pd_scores=predict_scores, gt_pos=gt_ids)
...
if __name__ == '__main__':
    #evaluate_retrieval_metric(candidates, gts, predict_scores=None, top_k=None, strategy=None):
    candidates = ['a|1', 'a|2', 'a|3', 'b|1', 'b|2', 'b|3']
    gts = ['a|2', 'b|1']
    print(evaluate_retrieval_metric(candidates,
                                    gts,
                                    top_k=3,
                                    strategy='next_chunk'))
